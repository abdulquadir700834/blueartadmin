...
import { useGetOneMediasInfoMutation, useSingleUserDetailsQuery, useUpdateMediaMutation } from "../Store/Store";
...
const MediaUpdate = () => {
  ...
  const [createPost2, responseInfo2] = useGetOneMediasInfoMutation()
  const [createPost, responseInfo] = useUpdateMediaMutation();
    ...
    onSubmit: (values) => {
      ...
      createPost(postData).then(res => {
        if (res?.data?.status !== false) {
          ...
          setTimeout(() => navigate("/media-publications"), 1500);
        } else {
