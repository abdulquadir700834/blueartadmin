...
const EditStyle = () => {
  ...
  const [editdates, responseInfo2] = useUpdateStyleSectionMutation();
  const [editCategory, responseInfo] = useEditStyleMutation();
  ...
  useEffect(() => {
    if (responseInfo?.data?.status) {
      ...
      setTimeout(() => navigate("/style"), 2000);
    } else {
            ...
            <div className='d-flex flex-wrap justify-content-end px-5'>
              <div className=''>
                <div className='btn-right'>
                  <Link className="btn btn-danger  btn-sm w-10" to="/style" >
