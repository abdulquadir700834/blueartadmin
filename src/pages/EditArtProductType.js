...
const EditArtProductType = () => {
  const { id } = useParams();
  let navigate = useNavigate();
  const [isLoading, setIsLoading] = useState(false);
  const [showCategory, setShowCategory] = useState({});
  const [editdates, responseInfo2] = useUpdateArtProductCategoryNameMutation();
  const [editCategory, responseInfo] = useEditArtProductCategoryNameMutation();
  ...
  const fetchData = async () => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_BACKEND_URL}UpdateArtProductImage`,
        formData,
        config
      )
      .then((res) => {
  ...
  useEffect(() => {
    if (responseInfo?.data?.status) {
      ...
      setTimeout(() => navigate("/artproducttype"), 2000);
    } else {
          ...
          <div className="row g-4 justify-content-center">
            <div className="d-flex flex-wrap justify-content-end px-5">
              <div className="">
                <div className="btn-right">
                  <Link
                    className="btn btn-danger  btn-sm w-10"
                    to="/artproducttype"
                  >
            ...
            <div className="col-12 col-lg-10 col-xl-9 col-xxl-8">
              <Tabs className="border-0 mb-3 settings-tabs">
                <Tab eventKey="" title="Edit Art Product Category Name">
                  <div className="card">
...
export default EditArtProductType;
