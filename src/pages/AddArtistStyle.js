...
const AddArtistStyle = () => {
  const [isLoading, setIsLoading] = useState(false);
  const [categoryTitle, setCategoryTitle] = useState("");
  const [categoryStatus, setCategoryStatus] = useState("Active");
  const [addMaterial, responseInfo] = useAddArtistStyleMutation();
  ...
  const handleSubmit = async (e) => {
    ...
    if (!categoryTitle) {
      setIsLoading(false)
      toast.error("Please enter a Artist Style");
      return;
    }
    addMaterial(newPost);
  }
                    ...
                    <div className="row g-4">
                      <div className="col-12">
                        <input
                          ...
                          id="name"
                          name="title"
                          placeholder="Your Artist Style Name..."
