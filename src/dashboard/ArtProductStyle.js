...
import { useGetArtProductStyleQuery } from "../Store/Store";
import PaginationNew from "../pages/PaginationNew";
import CryptoJS from 'crypto-js';

const ArtProductStyle = ({ module }) => {
    const [currentPage, setCurrentPage] = useState(1);
    const [searchTerm, setSearchTerm] = useState("");

    const { data, isLoading } = useGetArtProductStyleQuery({
        page: currentPage,
        search: searchTerm,
    });
                    ...
                    <div className="row">
                        ...
                        <div className='col-md-4 ms-auto'>
                            <div className='btn-right'>
                                <Link className="btn btn-warning rounded-pill btn-sm w-10" to="/add-artproductstyle">
                                    Add Art Product Style
                                </Link>
                            </div>
                        </div>
                    </div>
                                    ...
                                    currentPosts?.map((datas, index) => {
                                        ...
                                        <tr>
                                            ...
                                            <td>{datas.Status}</td>
                                            {
                                                module?.MaterialModule?.Write && (
                                                    <td className="text-center">
                                                        <Link to={`/edit-artproductstyle/${encodeURIComponent(encryptedItemId)}`}>
                                            ...
                                            }
                                            {
                                                module?.MaterialModule?.Read && (
                                                    <td className="text-center">
                                                        <Link to={`/view-artproductstyle/${encodeURIComponent(encryptedItemId)}`}>
...
export default ArtProductStyle;