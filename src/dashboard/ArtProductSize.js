...
import { useGetArtProductSizeQuery } from "../Store/Store";
import PaginationNew from "../pages/PaginationNew";
import Loader from "../components/loader/Loader";
import CryptoJS from 'crypto-js';

const ArtProductSize = ({ module }) => {
    const [currentPage, setCurrentPage] = useState(1);
    const [searchTerm, setSearchTerm] = useState("");
    const [postsPerPage, setPostsPerPage] = useState(10);

    const { data, isLoading } = useGetArtProductSizeQuery({
        page: currentPage,
        search: searchTerm,
        limit: postsPerPage,
    });
                    ...
                    <div className="row">
                        ...
                        <div className='col-md-4 ms-auto'>
                            <div className='btn-right'>
                                <Link className="btn btn-warning rounded-pill btn-sm w-10" to="/add-artproductsize">
                                    Add Art Product Size
                                </Link>
                            </div>
                        </div>
                    </div>
                                    ...
                                    currentPosts?.map((datas, index) => {
                                        ...
                                        <tr>
                                            ...
                                            <td>{datas.Status}</td>
                                            {
                                                module?.MaterialModule?.Write && (
                                                    <td className="text-center">
                                                        <Link to={`/edit-artproductsize/${encodeURIComponent(encryptedItemId)}`}>
                                            ...
                                            }
                                            {
                                                module?.MaterialModule?.Read && (
                                                    <td className="text-center">
                                                        <Link to={`/view-artproductsize/${encodeURIComponent(encryptedItemId)}`}>
