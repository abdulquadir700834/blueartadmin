...
import { useGetStyleQuery } from "../Store/Store";
import CryptoJS from 'crypto-js';

const Style = ({ module }) => {
    const [currentPage, setCurrentPage] = useState(1);
    const [searchTerm, setSearchTerm] = useState("");
    const { data, isLoading } = useGetStyleQuery({
        page: currentPage,
        search: searchTerm,
    });
                    ...
                    <div className="row">
                        ...
                        <div className='col-md-4 ms-auto'>
                            <div className='btn-right'>
                                <Link className="btn btn-warning rounded-pill btn-sm w-10" to="/add-style">
                                    Add Style
                                </Link>
                            </div>
                        </div>
                        ...
                        <table className="table">
                            <thead className="table-dark">
                                <tr>
                                    ...
                                    {
                                        module?.StylesModule?.Write &&
                                        <th scope="col" className="text-center">Edit</th>
                                    }
                                    {
                                        module?.StylesModule?.Read &&
                                    ...
                                    currentPosts?.map((datas, index) => {
                                        ...
                                        <tr>
                                            ...
                                            <td>{datas.Status}</td>
                                            {
                                                module?.StylesModule?.Write && (
                                                    <td className="text-center">
                                                        <Link to={`/edit-style/${encodeURIComponent(encryptedItemId)}`}>
                                            ...
                                            }
                                            {
                                                module?.StylesModule?.Read && (
                                                    <td className="text-center">
                                                        <Link to={`/view-style/${encodeURIComponent(encryptedItemId)}`}>
