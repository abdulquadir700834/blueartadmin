...
import { useGetExhibitionQuery } from "../Store/Store";
import Loader from "../components/loader/Loader";
import CryptoJS from 'crypto-js';

const Exhibition = ({ module }) => {
    const [currentPage, setCurrentPage] = useState(1);
    const [searchTerm, setSearchTerm] = useState("");

    const { data, isLoading } = useGetExhibitionQuery({
        page: currentPage,
        search: searchTerm,
    });
        ...
        <>
            ...
            <div className="admin-wrapper-table">
                <div className="container" >
                    <p style={{ fontSize: "2rem", color: "#fff", fontWeight: "800" }}>Exhibition</p>
                                        ...
                                        return (
                                            <tr>
                                                ...
                                                <td className="text-center">
                                                    <Link
                                                        to={`/update-exhibition/${encodeURIComponent(encryptedItemId)}`}
                                                    >
                                                        <i class="bi bi-pencil-fill text-white"></i>
                                                    </Link></td>
                                                <td className="text-center">
                                                    <Link
                                                        to={`/exhibition-list-info/${encodeURIComponent(encryptedItemId)}`}
                                                    >
