...
import { useGetNFTBlockchainInfoListQuery } from "../Store/Store";
import PaginationNew from "../pages/PaginationNew";

const NFTBlockchainInfo = ({ module }) => {
  // page
  const [currentPage, setCurrentPage] = useState(1);
  const [searchTerm, setSearchTerm] = useState("");

  const { data, isLoading, isError } = useGetNFTBlockchainInfoListQuery({
    page: currentPage,
    search: searchTerm,
  });
            ...
            <table className="table">
              <thead className="table-dark">
                <tr>
                  ...
                  {module?.NFTBlockchainModule?.Write && (
                    <th scope="col" className="text-center">
                      Edit
                    </th>
                  )}
                  {module?.NFTBlockchainModule?.Read && (
                    <th scope="col" className="text-center">
                      View
                    </th>
                  )}
                  ...
                  <tr>
                    ...
                    {module?.NFTBlockchainModule?.Write && (
                      <td className="text-center">
                        <Link to={`/edit-nft-info/${datas?._id}`}>
                    ...
                    {module?.NFTBlockchainModule?.Read && (
                      <td className="text-center">
                        <Link to={`/view-nft-info/${datas?._id}`}>
...
export default NFTBlockchainInfo;
