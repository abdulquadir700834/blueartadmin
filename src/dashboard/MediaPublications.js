...
import { useGetMediasListQuery } from "../Store/Store";
import CryptoJS from 'crypto-js';

const MediaPublications = ({ module }) => {
  const [currentPage, setCurrentPage] = useState(1);
  const [searchTerm, setSearchTerm] = useState("");
  const { data, isLoading } = useGetMediasListQuery({
    page: currentPage,
    search: searchTerm,
  });
    ...
    <>
      ...
      <div className="admin-wrapper-table">
        <div className="container" >
          <p style={{ fontSize: "2rem", color: "#fff", fontWeight: "800" }}>Media</p>
                    ...
                    return (
                      <tr>
                        ...
                        <td className="text-center">
                          <Link
                            to={`/update-media/${encodeURIComponent(encryptedItemId)}`}
                          >
                            <i class="bi bi-pencil-fill text-white"></i>
                          </Link></td>
                        <td className="text-center">
                          <Link
                            to={`/media-publications-list-info/${encodeURIComponent(encryptedItemId)}`}
                          >
