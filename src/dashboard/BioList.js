...
import { useGetBioListQuery } from "../Store/Store";
import CryptoJS from 'crypto-js';

const BioList = ({ module }) => {

    const [currentPage, setCurrentPage] = useState(1);
    const [searchTerm, setSearchTerm] = useState("");

    const { data, isLoading } = useGetBioListQuery({
        page: currentPage,
        search: searchTerm,
    });
    ...
    console.log("currentPostsBio", currentPosts)
        ...
        <>
            ...
            <div className="admin-wrapper-table">
                <div className="container" >
                    <p style={{ fontSize: "2rem", color: "#fff", fontWeight: "800" }}>Bio List</p>
                                        ...
                                        return (
                                            <tr>
                                                ...
                                                <td className="text-center">
                                                    <Link
                                                        to={`/update-bio-list/${encodeURIComponent(encryptedItemId)}`}
                                                    >
                                                        <i class="bi bi-pencil-fill text-white"></i>
                                                    </Link></td>
                                                <td className="text-center">
                                                    <Link
                                                        to={`/bio-list-info/${encodeURIComponent(encryptedItemId)}`}
                                                    >
