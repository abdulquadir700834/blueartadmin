...
import { useGetMaterialQuery } from "../Store/Store";
import Loader from "../components/loader/Loader";
import CryptoJS from 'crypto-js';

const Material = ({ module }) => {
                    ...
                    <div className="row">
                        ...
                        <div className='col-md-4 ms-auto'>
                            <div className='btn-right'>
                                <Link className="btn btn-warning rounded-pill btn-sm w-10" to="/add-material">
                                    Add Material
                                </Link>
                            </div>
                        </div>
                                    ...
                                    currentPosts?.map((datas, index) => {
                                        ...
                                        <tr>
                                            ...
                                            <td>{datas.Status}</td>
                                            {
                                                module?.MaterialModule?.Write && (
                                                    <td className="text-center">
                                                        <Link to={`/edit-material/${encodeURIComponent(encryptedItemId)}`}>
                                            ...
                                            }
                                            {
                                                module?.MaterialModule?.Read && (
                                                    <td className="text-center">
                                                        <Link to={`/view-material/${encodeURIComponent(encryptedItemId)}`}>
