...
import { useGetArtworkQuery } from "../Store/Store";
import CryptoJS from 'crypto-js';

const Artwork = ({ module }) => {
  const [currentPage, setCurrentPage] = useState(1);
  const [searchTerm, setSearchTerm] = useState("");

  const ArtData = useGetArtworkQuery({
    page: currentPage,
    search: searchTerm,
  });
...
if(ArtData.status == 'fulfilled'){
  setLoading(false)
  setPosts1(ArtData?.data)
}
console.log("postsArtData:", ArtData?.data)
}, [ArtData]);

  const totalCount = ArtData?.data?.count;
  const totalPages = Math.ceil(totalCount / postsPerPage);
  const currentPosts = posts1?.data?.filter((value) => {
    if (searchTerm === "") {
      return true;
    } else {
                  ...
                  return (
                    <tr>
                      ...
                      <td className="text-center">
                        <Link to={`/artwork-more-info/${encodeURIComponent(encryptedItemId)}`}>
